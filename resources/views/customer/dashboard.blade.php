<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
            ...
            <a href="{{ route('customer.profile') }}" class="inline-flex items-center px-3 sm:px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors duration-200">
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 sm:mb-8">
                <div class="p-4 sm:p-6 bg-gradient-to-r from-green-600 to-blue-600 text-white">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 sm:mb-8">
                <div class="p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                        <a href="{{ route('customer.packages') }}" class="bg-blue-600 text-white p-3 sm:p-4 rounded-lg text-center hover:bg-blue-700 transition duration-300 transform hover:scale-105">
                            <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>
                            <span class="font-medium text-sm sm:text-base">Browse Packages</span>
                        </a>
                        <a href="{{ route('contact') }}" class="bg-green-600 text-white p-3 sm:p-4 rounded-lg text-center hover:bg-green-700 transition duration-300 transform hover:scale-105">
                            <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                            </svg>
                            <span class="font-medium text-sm sm:text-base">Contact Support</span>
                        </a>
                        <a href="{{ route('profile.edit') }}" class="bg-purple-600 text-white p-3 sm:p-4 rounded-lg text-center hover:bg-purple-700 transition duration-300 transform hover:scale-105">
                            <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                            </svg>
                            <span class="font-medium text-sm sm:text-base">Update Profile</span>
                        </a>
                        <a href="{{ route('home') }}" class="bg-orange-600 text-white p-3 sm:p-4 rounded-lg text-center hover:bg-orange-700 transition duration-300 transform hover:scale-105">
                            <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
                            </svg>
                            <span class="font-medium text-sm sm:text-base">Go Home</span>
                        </a>
                    </div>
                </div>
            </div>

            <!-- Featured Packages for New Customers -->
            @if($totalBookings == 0)
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 sm:mb-8">
                <div class="p-4 sm:p-6">
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">Getting Started - Featured Packages</h3>
                    <p class="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Discover amazing Sri Lanka experiences and start your journey with these popular packages:</p>
                    
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                        @foreach($featuredPackages as $package)
                        <div class="bg-gray-50 rounded-lg p-3 sm:p-4 hover:shadow-md transition duration-300">
                            @if($package->image)
                                <div class="h-24 sm:h-32 bg-cover bg-center rounded mb-3" style="background-image: url('{{ asset('storage/' . $package->image) }}')">
                                    <div class="h-full w-full bg-black bg-opacity-30 rounded flex items-center justify-center">
                                        <span class="text-white font-bold text-sm sm:text-lg">{{ $package->location }}</span>
                                    </div>
                                </div>
                            @else
                                <div class="h-24 sm:h-32 bg-gradient-to-br from-blue-400 to-green-400 rounded mb-3 flex items-center justify-center">
                                    <span class="text-white font-bold text-sm sm:text-lg">{{ $package->location }}</span>
                                </div>
                            @endif
                            
                            <h4 class="font-semibold text-gray-900 mb-2 text-sm sm:text-base">{{ $package->name }}</h4>
                            <p class="text-xs sm:text-sm text-gray-600 mb-3">{{ Str::limit($package->description, 80) }}</p>
                            
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-xs sm:text-sm text-gray-500">{{ $package->duration }}</span>
                                <span class="text-sm sm:text-lg font-bold text-blue-600">${{ number_format($package->price, 2) }}</span>
                            </div>
                            
                            <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                                <a href="{{ route('customer.packages.show', $package) }}" 
                                    class="flex-1 bg-blue-600 text-white text-center py-2 px-3 rounded text-xs sm:text-sm hover:bg-blue-700 transition duration-300">
                                    View Details
                                </a>
                                <a href="{{ route('customer.packages.book', $package) }}" 
                                    class="flex-1 bg-green-600 text-white text-center py-2 px-3 rounded text-xs sm:text-sm hover:bg-green-700 transition duration-300">
                                    Book Now
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="text-center mt-4 sm:mt-6">
                        <a href="{{ route('customer.packages') }}" 
                            class="bg-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold hover:bg-blue-700 transition duration-300 inline-block text-sm sm:text-base">
                            View All Packages
                        </a>
                    </div>
                </div>
                                ...
                                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-4 lg:space-y-0">
                                    ...
                                    <div class="lg:ml-6 lg:text-right">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <a href="{{ route('customer.bookings.pdf', $booking) }}" class="block w-full bg-gradient-to-r from-green-500 to-green-600 text-white text-center py-2 px-4 rounded hover:from-green-600 hover:to-green-700 transition-all duration-300 text-xs sm:text-sm font-medium shadow-md hover:shadow-lg transform hover:scale-105">
